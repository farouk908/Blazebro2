...
import {
  ...
  AlertTriangle,
  Globe,
  Search, Truck, Box, Database, CloudLightning, Upload, Image
} from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Check hash on mount to support direct entry to secret pages (like #admin)
    const initialHash = window.location.hash;
    if (initialHash === '#admin') {
      setActiveView('admin');
    } else if (initialHash === '#shop' || initialHash.startsWith('#product-')) {
      setActiveView('shop');
    } else {
